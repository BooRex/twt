<?php 
	// -- ISSET func's
	function isset_login($user_login)
	{
		global $db_connect_link;

		$query = "SELECT COUNT(user_id) AS count FROM user WHERE user_login = '$user_login'";
	...
	function upd_user_image($user_id, $image_path)
	{
		global $db_connect_link;

		$query = "UPDATE user SET user_image = '$image_path' WHERE user_id = '$user_id'";
		$exec_query = mysqli_query($db_connect_link, $query);
		return true;
	}
	// is_ func's
		...
		if (is_uploaded_file($_FILES['user_image']['tmp_name']))
		{
			...
			if (preg_match("#(png|jpg|gif)#", $upl_image_extention))
			{
				if ($upl_image_size < IMAGE_MAX_SIZE)
				{
					...
					$upl_path = $upl_images_dir . md5($_SESSION['user_login']) . ".$upl_image_extention";
	...
	// -- OTHER func's
	function success_auth($user_login)
	{
		global $_SESSION;
		global $msg;
		...
		$_SESSION['user_id'] = get_user_id($user_login);
